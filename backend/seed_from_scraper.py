...
SCRAPER_OUTPUT_DIR = Path("../output")

def get_scraped_json_files():
    if not SCRAPER_OUTPUT_DIR.exists():
        print(f"Directory not found: {SCRAPER_OUTPUT_DIR}")
        return []
...
def load_recipe_from_json(json_path):
    try:
        ...
        # Extract real difficulty from scraper
        raw_diff = (
        ...
        return {
            ...
            "difficulty": difficulty,  # ← REAL difficulty from site
...
def seed_all_recipes():
    ...
    print(f"Found {len(json_files)} scraped recipe files.")
    ...
    with app.app_context():
        for json_path in json_files:
            ...
            if not recipe_data or not recipe_data["title"] or not recipe_data["source_url"]:
                skipped += 1
                continue
            ...
            try:
                ...
                for ing in recipe_data["ingredients"]:
                    ...
                    if isinstance(ing, dict):
                        ...
                        name = ing[0]
                        quantity = ing[1] if len(len(ing) > 1) else None
                ...
                for i, text in enumerate(instructions, 1):
                    if isinstance(text, dict):
                        text = text.get("text") or text.get("step")
                    if text:
                ...
                db.session.rollback()
                print(f"Failed: {recipe_data['title']}: {e}")
                skipped += 1
